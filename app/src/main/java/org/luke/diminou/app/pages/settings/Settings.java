...
public class Settings extends Titled {
    ...
    public Settings(App owner) {
        ...
        content.addView(new Setting(owner, "app_theme", Store::getTheme, v -> {
            Store.setTheme(v, owner::applyTheme);
        }, false, Style.THEME_SYSTEM, Style.THEME_DARK, Style.THEME_LIGHT));

        Setting scale = new Setting(owner, "ui_scale",
                Store::getScale,
                v -> Store.setScale(v, () -> ViewUtils.scale = Float.parseFloat(v)),
                true, "0.75", "1.0", "1.25");

        content.addView(scale);

        content.addView(new Setting(owner, "language", Store::getLanguage, v -> {
            Store.setLanguage(v, () -> {
        ...
        content.addView(new Setting(owner, "4_players_mode", Store::getFourMode, v -> {
            Store.setFourMode(v, null);
        }, false, FourMode.names()));

        content.addView(new Setting(owner, "play_timer", Store::getTimer, v -> {
            Store.setTimer(v, null);
        }, false, Timer.names()));
